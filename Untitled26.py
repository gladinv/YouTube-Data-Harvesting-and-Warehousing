...
def insert_into_videos():
    ...
    for i in collections1.find({"Channel_name" : user_inp},{'_id' : 0}):
        ...
        i["Tags"] = tags
        cur.execute(query1,tuple(i.values()))
        mydb.commit()
...
def execute_query(question_index):
    if question_index == 0:
        cur.execute("""SELECT title AS Video_Title, channel_name AS Channel_Name FROM videos ORDER BY channel_name""")
    elif question_index == 1:
        cur.execute("""SELECT channel_name AS Channel_Name, total_videos AS Total_Videos FROM channels ORDER BY total_videos DESC""")
    elif question_index == 2:
        cur.execute("""SELECT channel_name AS Channel_Name, title AS Video_Title, views AS Views FROM videos ORDER BY views DESC LIMIT 10""")
    elif question_index == 3:
        cur.execute("""SELECT a.video_id AS Video_id, a.title AS Video_Title, b.Total_Comments FROM videos AS a LEFT JOIN (SELECT video_id,COUNT(comment_id) AS Total_Comments FROM comments GROUP BY video_id) AS b ON a.video_id = b.video_id ORDER BY b.Total_Comments DESC""")
    elif question_index == 4:
        cur.execute("""SELECT channel_name AS Channel_Name,title AS Title,likes AS Likes_Count  FROM videos ORDER BY likes DESC LIMIT 3""")
    elif question_index == 5:
        cur.execute("""SELECT title AS Title, likes AS Likes_Count FROM videos ORDER BY likes DESC""")
    elif question_index == 6:
        cur.execute("""SELECT channel_name AS Channel_Name, views AS Views FROM channels ORDER BY views DESC""")
    elif question_index == 7:
        cur.execute("""SELECT channel_name AS Channel_Name FROM videos WHERE published_date LIKE '2022%' GROUP BY channel_name ORDER BY channel_name""")
        ...
        cur.execute("""SELECT channel_name AS Channel_Name, AVG(duration)/60 AS "Average_Video_Duration (mins)" FROM videos GROUP BY channel_name ORDER BY AVG(duration)/60 DESC""")
    elif question_index == 9:
        cur.execute("""SELECT channel_name AS Channel_Name,video_id AS Video_ID,comments AS Comments FROM videos ORDER BY comments DESC LIMIT 3""")
